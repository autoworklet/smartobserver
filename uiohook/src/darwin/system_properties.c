...
UIOHOOK_API screen_data* hook_create_screen_info(unsigned char *count) {
    ...
    *count = 0;
    ...
    CGDirectDisplayID *display_ids = (CGDirectDisplayID *)malloc(sizeof(CGDirectDisplayID) * UCHAR_MAX);
    if (display_ids != NULL) {
        ...
        if (status == kCGErrorSuccess && *count > 0) {
            ...
            screens = (screen_data *)malloc(sizeof(screen_data) * (*count));
            if (screens != NULL) {
                for (uint8_t i = 0; i < *count; i++) {
                    ...
                    if (boundsDisp.size.width > 0 && boundsDisp.size.height > 0) {
                        screens[i] = (screen_data) {
                            .number = (uint8_t)((uint8_t)i + (uint8_t)1),
                            ...
                            //negative)
                            .x = (int16_t)boundsDisp.origin.x,
                            .y = (int16_t)boundsDisp.origin.y,
                            .width = (uint16_t)boundsDisp.size.width,
                            .height = (uint16_t)boundsDisp.size.height
                        };
                }
                }
            }
            ...
            if (width > 0 && height > 0) {
                screens = (screen_data *)malloc(sizeof(screen_data));

...
static void initialize_modifiers() {
    ...
    if (CGEventSourceKeyState(kCGEventSourceStateCombinedSessionState, (CGKeyCode)kVK_Shift)) {
        set_modifier_mask(MASK_SHIFT_L);
    }
    if (CGEventSourceKeyState(kCGEventSourceStateCombinedSessionState, (CGKeyCode)kVK_RightShift)) {
        set_modifier_mask(MASK_SHIFT_R);
    }

    if (CGEventSourceKeyState(kCGEventSourceStateCombinedSessionState, (CGKeyCode)kVK_Control)) {
        set_modifier_mask(MASK_CTRL_L);
    }
    if (CGEventSourceKeyState(kCGEventSourceStateCombinedSessionState, (CGKeyCode)kVK_RightControl)) {
        set_modifier_mask(MASK_CTRL_R);
    }

    if (CGEventSourceKeyState(kCGEventSourceStateCombinedSessionState, (CGKeyCode)kVK_Option)) {
        set_modifier_mask(MASK_ALT_L);
    }
    if (CGEventSourceKeyState(kCGEventSourceStateCombinedSessionState, (CGKeyCode)kVK_RightOption)) {
        set_modifier_mask(MASK_ALT_R);
    }

    if (CGEventSourceKeyState(kCGEventSourceStateCombinedSessionState, (CGKeyCode)kVK_Command)) {
        set_modifier_mask(MASK_META_L);
    }
    if (CGEventSourceKeyState(kCGEventSourceStateCombinedSessionState, (CGKeyCode)kVK_RightCommand)) {
        set_modifier_mask(MASK_META_R);
    }

    if (CGEventSourceButtonState(kCGEventSourceStateCombinedSessionState, (CGMouseButton)kVK_LBUTTON)) {
        set_modifier_mask(MASK_BUTTON1);
    }
    if (CGEventSourceButtonState(kCGEventSourceStateCombinedSessionState, (CGMouseButton)kVK_RBUTTON)) {
        set_modifier_mask(MASK_BUTTON2);
    }

    if (CGEventSourceButtonState(kCGEventSourceStateCombinedSessionState, (CGMouseButton)kVK_MBUTTON)) {
        set_modifier_mask(MASK_BUTTON3);
    }
    if (CGEventSourceButtonState(kCGEventSourceStateCombinedSessionState, (CGMouseButton)kVK_XBUTTON1)) {
        set_modifier_mask(MASK_BUTTON4);
    }
    if (CGEventSourceButtonState(kCGEventSourceStateCombinedSessionState, (CGMouseButton)kVK_XBUTTON2)) {
...
static inline void process_key_pressed(uint64_t timestamp, CGEventRef event_ref) {
    ...
    event.data.keyboard.keychar = tis_keycode_message->buffer[0];
...
static inline void process_mouse_wheel(uint64_t timestamp, CGEventRef event_ref) {
    // printf("process_mouse_wheel\n");
    // Reset the click count and previous button.
    click_count = 1;
    click_button = MOUSE_NOBUTTON;

    int deltaAxis1 = CGEventGetIntegerValueField(event_ref, kCGScrollWheelEventDeltaAxis1);
    int deltaAxis2 = CGEventGetIntegerValueField(event_ref, kCGScrollWheelEventDeltaAxis2);
    ...
    // if (deltaAxis1 != 0 || deltaAxis2 != 0) {
        ...
        int pointDeltaAxis1 = CGEventGetIntegerValueField(event_ref, kCGScrollWheelEventPointDeltaAxis1);
        int pointDeltaAxis2 = CGEventGetIntegerValueField(event_ref, kCGScrollWheelEventPointDeltaAxis2);

        double preciseDelta1 = CGEventGetDoubleValueField(event_ref, kCGScrollWheelEventFixedPtDeltaAxis1);
        double preciseDelta2 = CGEventGetDoubleValueField(event_ref, kCGScrollWheelEventFixedPtDeltaAxis2);

        // printf("pointDeltaAxis1: %d\n", pointDeltaAxis1);
        // printf("pointDeltaAxis2: %d\n", pointDeltaAxis2);
        // printf("deltaAxis1: %d\n", deltaAxis1);
        // printf("deltaAxis2: %d\n", deltaAxis2);
        // printf("preciseDelta1: %f\n", preciseDelta1);
        // printf("preciseDelta2: %f\n", preciseDelta2);

        if (pointDeltaAxis1 != 0) {
            // event.data.wheel.amount = pointDeltaAxis1 / deltaAxis1;
            event.data.wheel.amount = abs(pointDeltaAxis1);
            // sign(pointDeltaAxis1)
            event.data.wheel.rotation = pointDeltaAxis1 > 0 ? 1 : -1;
            event.data.wheel.direction = WHEEL_VERTICAL_DIRECTION;
        } else if (pointDeltaAxis2 != 0) {
            // Implementing horizontal scrolling by examining axis 2
            // event.data.wheel.amount = pointDeltaAxis2 / deltaAxis2;
            event.data.wheel.amount = abs(pointDeltaAxis2);
            event.data.wheel.rotation = pointDeltaAxis2 > 0 ? 1 : -1;
            event.data.wheel.direction = WHEEL_HORIZONTAL_DIRECTION;
        } else {
            // In case a 3rd axis is introduced without updating this code.
            event.data.wheel.amount = 0;
            event.data.wheel.rotation = 0;
        }
...
static int create_event_runloop_info(event_runloop_info **hook) {
    ...
    // Try and allocate memory for event_runloop_info.
    *hook = (event_runloop_info *)calloc(1, sizeof(event_runloop_info));
    if (*hook == NULL) {

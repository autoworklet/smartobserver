...
// Global variable to track if the worker is started.
...
int hook_enable() {
    ...
    if (pthread_create(&hook_thread, &hook_thread_attr, (void *(*)(void *)) hook_thread_proc, hook_thread_status) == 0) {
    #endif
        #if defined(_WIN32)
        // Attempt to set the thread priority to time critical.
...
int uiohook_worker_start(dispatcher_t dispatch_proc) {
  ...
  if (status == UIOHOOK_SUCCESS) {
      worker_started = 1; // Indicate that the worker has successfully started
  }
...
int uiohook_worker_stop() {
  ...
  if (status == UIOHOOK_SUCCESS) {
    ...
    worker_started = 0; // Indicate that the worker has successfully stopped

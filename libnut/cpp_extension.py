...
from torch.utils.cpp_extension import _HERE
...
def _write_ninja_file(path,
    ...
    # replace "-std=c++17" with "" in cflags
    cflags_mm = cflags.copy()
    cflags_mm = [x for x in cflags_mm if x != "-std=c++17"]
    flags.append(f'cflags_mm = {" ".join(cflags_mm)}')
    ...
    compile_mm_rile = ['rule compile_mm']
    if IS_WINDOWS:
        compile_mm_rile.append(
            '  command = cl /showIncludes $cflags -c $in /Fo$out $post_cflags')
        compile_mm_rile.append('  deps = msvc')
    else:
        compile_mm_rile.append(
            '  command = $cxx -MMD -MF $out.d $cflags_mm -c $in -o $out $post_cflags')
        compile_mm_rile.append('  depfile = $out.d')
        compile_mm_rile.append('  deps = gcc')
    ...
    for source_file, object_file in zip(sources, objects):
        ...
        source_file = source_file.replace(" ", "$ ")
        object_file = object_file.replace(" ", "$ ")
        # if source_file include .m or .mm use compile_mm rule
        if source_file.endswith('.m') or source_file.endswith('.mm'):
            build.append(f'build {object_file}: compile_mm {source_file}')
        else:
    ...
    blocks = [config, flags, compile_rule, compile_mm_rile]
    if with_cuda:

...
void toggleKeyCode(MMKeyCode code, const bool down, MMKeyFlags flags) {
  ...
  if (code >= 1000) {
    ...
    if (code == K_FUNCTION) {
      flags = (MMKeyFlags)(flags | MOD_FN);
    }

    MMKeyFlags activeKeyFlags;
    if (down) {
      activeKeyFlags = (MMKeyFlags)(flags | flagBuffer);
      flagBuffer = (MMKeyFlags)(flagBuffer | activeKeyFlags);
    } else {
      activeKeyFlags = (MMKeyFlags)(flags ^ flagBuffer);
      flagBuffer = (MMKeyFlags)(flagBuffer ^ flags);
    }
...
void toggleKey(char c, const bool down, MMKeyFlags flags) {
  ...
  if (isupper(c) && !(flags & MOD_SHIFT)) {
    flags = (MMKeyFlags)(flags | MOD_SHIFT); /* Not sure if this is safe for all layouts. */
  }
...
void toggleUnicodeKey(unsigned long ch, const bool down) {
  ...
  if (ch > 0xFFFF) {
    ...
    unsigned short surrogates[] = {(unsigned short)(0xD800 + ((ch - 0x10000) >> 10)),
                                   (unsigned short)(0xDC00 + (ch & 0x3FF))};

    CGEventKeyboardSetUnicodeString(keyEvent, 2, surrogates);
  } else {
    CGEventKeyboardSetUnicodeString(keyEvent, 1, (const UniChar *)&ch);
  }
